...
RedactionConfig, MaskConfig, Nightfall
from os import walk
from nightfall.alerts import AlertConfig, EmailAlert
nightfall = Nightfall() # reads API key from NIGHTFALL_API_KEY environment 
variable by default
...
# else use a default inline detection rule for credit card numbers, SSNs, 
and API keys
...
if detection_rule_uuid is None:
	print("No detection rule UUID specified - using default inline 
detection rule")
	detection_rules = [ DetectionRule([ # specify an inline detection 
rule
		Detector(
